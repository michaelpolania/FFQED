...
void Compute_J(VectorField & B, VectorField & J, size_t N_GC, const Domain & dm)
{
    ...
    for(size_t i=N_GC; i<B.shape()[1]-N_GC; i++){
        ...
        for(size_t j=N_GC; j<B.shape()[2]-N_GC; j++){
            ...
            // Calculation of A_1_z

            double rho_avg_A_1_z = 0.5 * (Rho[i][j] + Rho[i-1][j] + Rho[i][j-1] + Rho[i-1][j-1]);
    
            double Ex_By_A_1_z = 0.5 * (E[0][i][j] * B[1][i][j] + E[0][i-1][j] * B[1][i-1][j]);   

            double Ey_Bx_A_1_z = 0.5 * (E[1][i][j] * B[0][i][j] + E[1][i][j-1] * B[0][i][j-1]);

            double Ez_A_1_y = 0.5 * (E[2][i][j] + E[2][i][j+1]);

            double Bx_i = 0.5 * (B[0][i+1][j] + B[0][i][j]);
            double Bx_i_minus_1_j = 0.5 * (B[0][i-1][j] + B[0][i][j]);
            double Bx_i_j_minus_1 = 0.5 * (B[0][i+1][j-1] + B[0][i][j-1]);
            double Bx_ij_minus_1 = 0.5 * (B[0][i-1][j-1] + B[0][i][j-1]);
            
            double By_j = 0.5 * (B[1][i][j+1] + B[1][i][j]);
            double By_i_minus_1_j = 0.5 * (B[1][i-1][j+1] + B[1][i-1][j]); 
            double By_i_j_minus_1 = 0.5 * (B[1][i][j] + B[1][i][j-1]); 
            double By_ij_minus_1 = 0.5 * (B[1][i-1][j] + B[1][i-1][j-1]);
            
            double Bz = B[2][i][j];
            double Bz_i_minus_1_j = B[2][i-1][j];
            double Bz_i_j_minus_1 = B[2][i][j-1];
            double Bz_ij_minus_1 = B[2][i-1][j-1];

            double B_ij = (Bx_i * Bx_i)/4.0  + (By_j * By_j)/4.0  + (Bz * Bz); 
            double B_i_minus_1_j = (Bx_i_minus_1_j * Bx_i_minus_1_j)/4.0 +(By_i_minus_1_j * By_i_minus_1_j)/4.0 + (Bz_i_minus_1_j * Bz_i_minus_1_j);
            double B_i_j_minus_1 = (Bx_ij_minus_1 * Bx_ij_minus_1)/4.0 + (By_i_j_minus_1 * By_i_j_minus_1)/4.0 + (Bz_i_j_minus_1 * Bz_i_j_minus_1);
            double B_ij_minus_1 = (Bx_ij_minus_1 * Bx_ij_minus_1)/4.0 + (By_ij_minus_1 * By_ij_minus_1)/4.0 + (Bz_ij_minus_1 * Bz_ij_minus_1);


            double ExB_z = Ex_By_A_1_z  - Ey_Bx_A_1_z;

            double A_1_y = (4.0 * rho_avg_A_1_z * ExB_z)/(B_ij+ B_i_minus_1_j + B_i_j_minus_1 + B_ij_minus_1); // check w/ Peter on factor of 8
            ...
            J[0][i][j] = A_1_x + A_2_x + A_3_x;
            J[1][i][j] = ( B[2][i-1][j] - B[2][i][j] )/Deltax_n1;
            J[2][i][j] = ( B[1][i][j] - B[1][i-1][j] )/Deltax_n1 - ( B[0][i][j] - B[0][i][j-1] )/dm.Deltay;
        }
    }
